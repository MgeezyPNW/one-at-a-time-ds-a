...
const forLoopTwoToThe = (arr) => {
  ...
  const results = [];
  for (let num in arr) {
    results.push(Math.pow(2, arr[num]));
  }
  return results;
};
...
const extractAbilities = arr => {
  // Solution code here...
  return arr.map(array => array.ability.name);
};
...
const extractStats = (arr) => {
  // Solution code here...
  return arr.map(array => ({ name: array.stat.name, total: array.effort + array.baseStat }));
};
